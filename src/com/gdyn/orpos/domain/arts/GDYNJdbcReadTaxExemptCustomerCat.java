...
public class GDYNJdbcReadTaxExemptCustomerCat extends JdbcDataOperation
        implements GDYNARTSDatabaseIfc
{
    ...
    protected static final Logger log =
            Logger.getLogger(GDYNJdbcReadTaxExemptCustomerCat.class);
            ...
            throws DataException
    {
        ...
        GDYNHandleReturnPOJO searchObj = (GDYNHandleReturnPOJO) action.getDataObject();
        ...
        sql.addColumn(ALIAS_TAX_EXEMPT_CUST_CATEGORY + "." + FIELD_CATEGORY_CODE); //1
        sql.addColumn(ALIAS_TAX_EXEMPT_CUST_CATEGORY + "." + FIELD_COUNTRY_CODE);//2
        sql.addColumn(ALIAS_TAX_EXEMPT_CUST_CATEGORY + "." + FIELD_TAX_AREA_CODE);//3
        sql.addColumn(ALIAS_TAX_EXEMPT_CUST_CATEGORY + "." + FIELD_CATEGORY_MSG_ID);//4
        sql.addColumn(ALIAS_TAX_EXEMPT_CUST_CATEGORY + "." + FIELD_ACTIVE_FLAG);//5
        sql.addColumn(ALIAS_TAX_EXEMPT_CUST_CATEGORY + "." + FIELD_EXPIRE_DATE_REQ_FLAG);//6
        sql.addColumn(ALIAS_TAX_EXEMPT_CUST_CATEGORY + "." + FIELD_BAND_REGISTRY_REQ_FLAG);//7
        sql.addColumn(ALIAS_TAX_EXEMPT_CUST_CATEGORY + "." + FIELD_TAX_ID_IMAGE_REQ_FLAG);//8

        sql.addQualifier(ALIAS_RETAIL_STORE + "." + FIELD_RETAIL_STORE_ID + " = " + searchObj.getStoreNum());
        sql.addQualifier(ALIAS_TAX_EXEMPT_CUST_CATEGORY + "." + FIELD_ACTIVE_FLAG + " = " + "'1'");
        sql.addQualifier(ALIAS_TAX_EXEMPT_CUST_CATEGORY + "." + FIELD_CATEGORY_CODE + " = " + makeSafeString(searchObj.getTaxExemptCategoryCode()));
        ...
        sql.addQualifier(ALIAS_TAX_EXEMPT_CUST_CATEGORY + "." + FIELD_TAX_AREA_CODE + " = " + ALIAS_ADDRESS + "." + FIELD_CONTACT_STATE );
        //
        ...
        logger.error("Mansi SQLToString: " + sql.getSQLString());
        ...
        try
        {
            ...
            while (rs.next())
            {
                 ...
                 int index = 0;
                 String CategoryCode =getSafeString(rs, 1);
                 category.setCategoryCode(CategoryCode);
                 String CountryCode =getSafeString(rs, 2);
                 category.setCountryCode(CountryCode);
                 String TaxAreaCode =getSafeString(rs, 3);
                 category.setTaxAreaCode(TaxAreaCode);
                 Integer CategoryMessageId = (rs.getInt(4));
                 category.setCategoryMessageId(CategoryMessageId);
                 boolean Active = getBooleanFromString(rs, 5);
                 category.setActive(Active);
                 boolean ExpirationDateRequired =getBooleanFromString(rs, 6);
                 category.setExpirationDateRequired(ExpirationDateRequired);
                 boolean BandRegistryRequired =getBooleanFromString(rs, 7);
                 category.setBandRegistryRequired(BandRegistryRequired);
                 boolean TaxIdImageRequired =getBooleanFromString(rs, 8);
                 category.setTaxIdImageRequired(TaxIdImageRequired);
        ...
        logger.error("Mansi after categoryList size: " + categoryList.size());
        if(categoryList != null && categoryList.size() == 1)
        {
        	GDYNTaxExemptCustomerCategory category = categoryList.get(0);
        	
			readCategoryInternationalizedValues(connection, category);
            
        	if (searchObj.isPopulate())
            {
                GDYNTaxExemptCustomerCode code =
                        (GDYNTaxExemptCustomerCode) selectCustomerCodeByCategory(
                        		connection, category.getCategoryCode(),
                                category.getCountryCode(), category.getTaxAreaCode(), searchObj.isPopulate());
               
                category.setCustomerCode(code);
               
            }
        	GDYNTransactionTaxIfc transactionTax = searchObj.getTxnTax();
        	transactionTax.setCustomerCategory(category);
        	
        	dataTransaction.setResult(transactionTax);
        }
        else
        {
