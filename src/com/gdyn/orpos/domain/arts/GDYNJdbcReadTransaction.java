  ...
  implements GDYNARTSDatabaseIfc
{
  ...
  protected void selectSaleReturnTransaction(JdbcDataConnection dataConnection, SaleReturnTransactionIfc transaction, LocaleRequestor localeRequestor, boolean retrieveStoreCoupons) throws DataException
  {
    ...
    // modified by Vivek to add the Loyalty Transaction table while reading the transaction Currently adding only when the TYPE is RETURN
    if(transaction.getTransactionType() == TransactionConstantsIfc.TYPE_RETURN){
    	sql.addTable(TABLE_LOYALTY_TRANSACTION_RECORD, ALIAS_LOYALTY_TABLE);
    }
    ...
    //modified by vivek : added the column for Loyalty table
    if(transaction.getTransactionType() == TransactionConstantsIfc.TYPE_RETURN) {
    	sql.addColumn(ALIAS_LOYALTY_TABLE + "." + FIELD_LOYALTY_ID);
    	sql.addColumn(ALIAS_LOYALTY_TABLE + "." + FIELD_LOYALTY_EMAIL_ID);
    }
    ...
    try
    {
        ...
        // modified by vivek : get the Loyalty/Email to the String
        if(transaction.getTransactionType() == TransactionConstantsIfc.TYPE_RETURN) {
            ...
            if (transaction instanceof GDYNSaleReturnTransactionIfc)
      {
        ...
        //add the loyalty/Email to the transaction Object
        if(transaction.getTransactionType() == TransactionConstantsIfc.TYPE_RETURN) {
