...
// End GD-237: GD_Item lookup mismatch columns when viewing items returned in item lookup by item description
{
            ...
            throws DataException
    {
        ...
        /*for(PLUItemIfc pluItem : items){
        	logger.info("Before Calling readCpnAttrDtls method ");
        	readCpnAttrDtls(connection, pluItem.getItemID());
        }*/
            ...
            throws DataException
    {
    	 ...
    	 logger.info("inside readCpnAttrDtls GDYNJdbcReadItemInfo");
    	 //System.out.println("inside readCpnAttrDtls GDYNJdbcReadItemInfo");
        ...
        {
            String sqlToRun = sql.getSQLString();
            logger.info("sqlQuery for coupon To Run from GDYNJdbcReadItemInfo "+sqlToRun);
            dataConnection.execute(sqlToRun);
            ResultSet rs = (ResultSet) dataConnection.getResult();
            
            while (rs.next())
            {
            	logger.info("Details from readCpnAttrDtls  "+getSafeString(rs, 1)+" : "+getSafeString(rs, 2)+" : "+getSafeString(rs, 3)+ " : "+getSafeString(rs,4)); 
